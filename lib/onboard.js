...
const REPO_URL = "https://github.com/zasanao/lurk.git";
...
async function onboard(args) {
  ...
  const pyVersion = run("python3 -c 'import sys; print(f\"{sys.version_info.major}.{sys.version_info.minor}\")'");
  ...
  const swiftVersion = swiftVersionRaw.replace(/.*version\s+/, "").split(" ")[0];
  ...
  if (inRepo) {
    ...
    run(`git clone --depth 1 "${REPO_URL}" "${LURK_SRC}"`, { stdio: "inherit" });
    srcDir = LURK_SRC;
  ...
  for (const { spec, label } of extras) {
    ...
    if (injectResult.status === 0) {
      ...
      warn(`${label} — failed to install, add later with: pipx inject lurk "${spec}"`);
    }
  }
  ...
  console.log("");
  console.log(
    `  lurk needs Accessibility access to read window titles.`
  );
  console.log(
  ...
  console.log(`  ${CYAN}claude mcp add lurk -- lurk serve-mcp${RESET}   Claude Code`);
  console.log(`  ${CYAN}lurk serve-http${RESET}                        HTTP API at :4141`);
  console.log("");
}
