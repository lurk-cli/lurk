...
async function onboard(args) {
  ...
  // ── Step 4.5: Ensure Ollama is available ──

  step("Setting up local LLM (Ollama)...");

  if (which("ollama")) {
    ok("Ollama already installed");
  } else if (which("brew")) {
    warn("Ollama not found — installing via Homebrew...");
    const ollamaResult = spawnSync("brew", ["install", "ollama"], {
      stdio: "inherit",
      env: process.env,
    });
    if (ollamaResult.status === 0) {
      ...
      warn("Ollama install failed — lurk will work without LLM synthesis");
    }
  } else {
    warn("Ollama not found — install it from https://ollama.ai for enhanced context");
  }

  // Start Ollama and pull the model
  if (which("ollama")) {
    // Start the Ollama service (runs in background)
    spawnSync("ollama", ["serve"], {
      stdio: "ignore",
      detached: true,
    });
    ...
    console.log(`  Pulling llama3.2:3b model (this may take a few minutes)...`);
    const pullResult = spawnSync("ollama", ["pull", "llama3.2:3b"], {
      stdio: "inherit",
      env: process.env,
    ...
    if (pullResult && pullResult.status === 0) {
      ...
      warn("Model pull failed — lurk will work without LLM synthesis");
    }
  }
